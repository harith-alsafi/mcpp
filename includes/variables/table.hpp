...
namespace var
{
	...
	class table
	{
	  ...
	  public:
		...
		void set_col_name(int index, std::string names)
		{
			if(index < 0 || index >= _col) {
		...
		void insert_col(std::vector<D> a, int j)
		{
			if(j < 0 || j > _col){
			...
			_col++;
			for(int i = 0; i < _row; i++) {
				data.insert(data.begin() + get_index(i, j), col[i]);
			}
		}
