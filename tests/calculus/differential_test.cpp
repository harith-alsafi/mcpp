...
TEST(diff, fun)
{
    ...
    auto sds2 = vect::round(ansdy1);
    ASSERT_TRUE(vect::round(ansdy1) == dy1);

    // 2nd derivative
    std::vector<double> dy2 = {3, 3, 5};
    auto ansdy2 = diff(f, x, 2);
    ASSERT_TRUE(vect::round(ansdy2) == dy2);

    // 3rd derivative
    std::vector<double> dy3 = {-1, 2};
    auto ansdy3 = diff(f, x, 3);
    ASSERT_TRUE(vect::round(ansdy3) == dy3);

    // 3rd derivative
    std::vector<double> dy4 = {3};
    auto ansdy4 = diff(f, x, 4);
    ASSERT_TRUE(vect::round(ansdy4) == dy4);
